...
const generateProfessionalPdf = (doc, bill) => {
  try {
    ...
    doc.fillColor(primaryColor)
      .fontSize(20)
      .font("Helvetica-Bold")
      .text("TAX INVOICE", pageWidth - margin - 150, currentY, { align: "right" })
      .fontSize(10)
      .text(`Invoice #: ${billNumber}`, pageWidth - margin - 150, currentY + 25, { align: "right" })
      .text(`Date: ${moment(createdAt).format("DD/MM/YYYY")}`, pageWidth - margin - 150, currentY + 40, { align: "right" })
      .text(`Time: ${moment(createdAt).format("hh:mm A")}`, pageWidth - margin - 150, currentY + 55, { align: "right" });
    ...
    customers.forEach((customer, customerIndex) => {
      ...
      if (customerItems.length > 0) {
        ...
        const rowHeight = 20;
        const colWidths = [200, 60, 80, 70, 90, 80];
        ...
        doc.fillColor(textColor)
          .fontSize(10)
          .font("Helvetica-Bold")
          .text(`Subtotal for ${customerName}:`, margin + contentWidth - 210, currentY + 8)
          .text(`₹${Number(customerSubtotal).toFixed(2)}`, margin + contentWidth - 30, currentY + 8, { align: "right" });
    ...
    doc.rect(totalsX - 20, currentY, totalsWidth + 20, 90)
      .fill(backgroundColor)
      .stroke(borderColor);
    ...
    doc.fillColor(textColor)
      .fontSize(10)
      .font("Helvetica")
      .text("Total Amount:", totalsX, totalsY)
      .text(`₹${Number(totalAmount).toFixed(2)}`, totalsX + 200, totalsY, { align: "right" });

    totalsY += 15;

    doc.text("Total Commission:", totalsX, totalsY)
      .text(`₹${Number(totalCommission).toFixed(2)}`, totalsX + 200, totalsY, { align: "right" });

    totalsY += 15;

    if (discountPercent > 0) {
      doc.text(`Discount (${discountPercent}%):`, totalsX, totalsY)
        .text(`-₹${Number(discountAmount).toFixed(2)}`, totalsX + 200, totalsY, { align: "right" });
      totalsY += 15;
    }
    ...
    doc.fillColor(primaryColor)
      .fontSize(12)
      .font("Helvetica-Bold")
      .text("FINAL AMOUNT:", totalsX, totalsY)
      .text(`₹${Number(finalAmount).toFixed(2)}`, totalsX + 200, totalsY, { align: "right" });
